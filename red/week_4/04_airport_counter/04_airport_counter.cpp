...
public:
  ...
  AirportCounter() {
    for (size_t i = 0; i < airports.size(); ++i)
      airports[i] = {static_cast<TAirport>(i), 0};
  };
  ...
  AirportCounter(TIterator begin, TIterator end) {
    for (size_t i = 0; i < airports.size(); ++i)
      airports[i] = {static_cast<TAirport>(i), 0};
    while (begin != end) {
      ++airports[GetAirportNum(*begin)].second;
      ++begin;
    }
  }
  ...
  size_t Get(TAirport airport) const {
    return airports[GetAirportNum(airport)].second;
  }
  
  // добавить данный элемент
  void Insert(TAirport airport) {
    ++airports[GetAirportNum(airport)].second;
  }
  ...
  void EraseAll(TAirport airport) {
    airports[GetAirportNum(airport)].second = 0;
  }
...
private:
  [[nodiscard]]
  size_t GetAirportNum(TAirport airport) const {
    return static_cast<uint32_t>(airport);
  }
  std::array<Item, static_cast<uint32_t>(TAirport::Last_)> airports;
};
